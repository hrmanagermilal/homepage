...
class HeroController extends BaseController {
    ...
    public function index(): void {
        ...
        $linksRaw  = $this->heroModel->getLinks();
        // 컬럼명 매핑
        $links = array_map(function($lk) {
            $lk['icon_url'] = $lk['image'] ?? null;
            $lk['link_url'] = $lk['link']  ?? null;
            return $lk;
        }, $linksRaw);
            $heroes = [];
    ...
    public function create(): void {
        ...
        // DB 구조상 단일 히어로만 사용
        $this->success(['id' => 1], '히어로가 추가되었습니다.');
    }
    ...
    public function delete(): void {
        ...
        $id = $this->intPost('id');
        foreach ($this->heroModel->getBgImages($id) as $bg)
            UploadHelper::deleteFile($bg['image_url']);
    ...
    public function linkList(): void {
        ...
        // quick_links 컬럼명을 View가 기대하는 키로 매핑
        $links = array_map(function($lk) {
            $lk['icon_url'] = $lk['image'] ?? null;
            $lk['link_url'] = $lk['link']  ?? null;
            return $lk;
        }, $links);
            $this->success(['links' => $links]);
    }
    ...
    public function linkDetail(): void {
        ...
        // View에서 icon_url, link_url 키를 기대하므로 매핑
        $row['icon_url'] = $row['image'] ?? null;
        $row['link_url'] = $row['link']  ?? null;
        $this->success($row);
    }
    ...
    public function linkDelete(): void {
        ...
        $row = $this->heroModel->findLink($id);
        if (!$row) $this->error('링크를 찾을 수 없습니다.', 404);
        if (!empty($row['image'])) UploadHelper::deleteFile($row['image']);
    ...
    public function addBgImage(): void {
        ...
        if (empty($_FILES['image']) || $_FILES['image']['error'] !== UPLOAD_ERR_OK)
            $this->error('이미지를 선택해주세요.');
            $upload = UploadHelper::uploadImage($_FILES['image'], 'heroes');
            if (!$upload['success']) $this->error($upload['message']);
            $count = count($this->heroModel->getBgImages($heroId));
            $id    = $this->heroModel->addBgImage($heroId, $upload['path'], $count + 1, $this->post('alt_text', ''));
            $this->success(['id' => $id, 'image_url' => $upload['url']], '배경 이미지가 추가되었습니다.');
    }
    
    // View에서 호출하는 엔드포인트명 매핑
    public function bgImageAdd(): void    { $this->addBgImage(); }
    ...
    // View에서 호출하는 엔드포인트명 매핑
    public function bgImageDelete(): void { $this->deleteBgImage(); }
    ...
    // View에서 호출하는 엔드포인트명 매핑
    public function bgImageReorder(): void { $this->reorderBgImages(); }
    ...
    // View에서 호출하는 엔드포인트명 매핑
    public function frontImageUpsert(): void { $this->upsertFrontImage(); }
    ...
    // View에서 호출하는 엔드포인트명 매핑
    public function frontImageDelete(): void { $this->deleteFrontImage(); }
}
