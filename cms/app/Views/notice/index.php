    ...
    <table>
      ...
      <tbody id="notice-tbody">
        ...
        <tr data-id="<?= $n['id'] ?>" data-image="<?= htmlspecialchars($n['image']??'') ?>">
          ...
          <td>
            <?php if($n['image']): ?>
            <button class="img-thumb-btn" onclick="showImgPopup('<?= BASE_URL.htmlspecialchars($n['image']) ?>')" title="이미지 보기">
              <img src="<?= BASE_URL.htmlspecialchars($n['image']) ?>" class="img-thumb-mini" onerror="this.parentElement.innerHTML='<i class=\'fas fa-image text-muted\'></i>'">
            </button>
