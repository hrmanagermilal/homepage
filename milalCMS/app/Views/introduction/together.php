  ...
  <div id="together-list" class="sortable-list">
    ...
    <div class="together-item" data-id="<?= $item['id'] ?>">
      ...
      <div class="together-logo">
        <?php if($item['image']): ?>
        <img src="<?= UPLOAD_URL.htmlspecialchars($item['image']) ?>" alt="<?= htmlspecialchars($item['title']) ?>" style="height:48px;object-fit:contain;max-width:120px;" onerror="this.style.display='none'">
        <?php else: ?>
...
async function editItem(id) {
  ...
  if(row.image){curI.src='<?= UPLOAD_URL ?>'+row.image;curW.style.display='';}
