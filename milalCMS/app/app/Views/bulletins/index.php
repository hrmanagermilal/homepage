...
async function saveEdit(){
  ...
  if(d.success){
    ...
    if(tr){
      const titleEl=tr.querySelector('td:first-child a');
      if(titleEl) titleEl.textContent=document.getElementById('bul-edit-title').value;
      const yearEl=tr.querySelector('td:nth-child(2)');
      if(yearEl) yearEl.textContent=document.getElementById('bul-edit-year').value||'-';
      const weekEl=tr.querySelector('td:nth-child(3)');
      const weekVal=document.getElementById('bul-edit-week').value;
      if(weekEl) weekEl.textContent=weekVal?weekVal+'주차':'-';
    }
