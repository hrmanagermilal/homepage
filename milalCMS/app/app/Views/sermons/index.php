...
async function saveSermon(){
  ...
  if(d.success){
    ...
    const id2=document.getElementById('sermon-id').value;
    if(id2){
      const tr=document.querySelector(`tr[data-id="${id2}"]`);
      if(tr){
        const a=tr.querySelector('td:nth-child(2) a');
        if(a) a.textContent=document.getElementById('s-title').value;
        const preacher=tr.querySelector('td:nth-child(3)');
        if(preacher) preacher.textContent=document.getElementById('s-preacher').value||'-';
        const dateEl=tr.querySelector('td:nth-child(4)');
        if(dateEl) dateEl.textContent=document.getElementById('s-date').value||'-';
      }
