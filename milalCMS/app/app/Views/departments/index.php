...
async function saveDept(){
  ...
  if(d.success){
    toast(d.message);closeModal('dept-modal');
    if(id){
      const tr=document.querySelector(`tr[data-id="${id}"]`);
      if(tr){
        const nameEl=tr.querySelector('td:nth-child(2) a');
        if(nameEl) nameEl.textContent=document.getElementById('d-name').value;
        const typeVal=document.getElementById('d-type').value;
        const typeBadge=tr.querySelector('td:nth-child(3) .badge');
        if(typeBadge){typeBadge.className='badge '+(typeVal==='nextgen'?'badge-blue':'badge-purple');typeBadge.textContent=typeVal==='nextgen'?'다음세대':'사역';}
      }
