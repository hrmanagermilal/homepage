...
$canCreate = hasPerm('departments.create');
?>
...
<div style="display:grid;grid-template-columns:300px 1fr;gap:20px;align-items:start">
  ...
  <div class="card">
    <div class="card-body" style="padding:24px;text-align:center">
      <?php if($dept['image']): ?>
      <img src="<?= UPLOAD_URL.htmlspecialchars($dept['image']) ?>"
           style="width:100%;max-height:180px;object-fit:cover;border-radius:8px;margin-bottom:14px" alt="">
      <?php else: ?>
      <div style="width:80px;height:80px;border-radius:50%;background:var(--primary);display:flex;align-items:center;justify-content:center;margin:0 auto 14px;font-size:28px;color:#fff">
      ...
      <div style="margin:8px 0">
        <span class="badge <?= $dept['department_type']==='nextgen' ? 'badge-blue' : 'badge-purple' ?>">
          <?= $dept['department_type']==='nextgen' ? '다음세대' : '사역부서' ?>
        </span>
        <span class="badge <?= $dept['is_active'] ? 'badge-green' : 'badge-gray' ?>" style="margin-left:4px">
          <?= $dept['is_active'] ? '활성' : '비활성' ?>
        </span>
      </div>
      <?php if($dept['description']): ?>
      <p style="font-size:13px;color:var(--text-muted);margin-top:8px"><?= htmlspecialchars($dept['description']) ?></p>
      <?php endif; ?>
    </div>
    <div style="padding:0 16px 16px;display:flex;flex-direction:column;gap:8px">
      <?php if($canEdit): ?>
      <button class="btn btn-warning" onclick="openEdit()"><i class="fas fa-pen"></i> 수정</button>
      <?php endif; ?>
      <?php if($canDelete): ?>
      <button class="btn btn-danger" onclick="deleteDept(<?= $dept['id'] ?>)"><i class="fas fa-trash"></i> 삭제</button>
      <?php endif; ?>
    </div>
  </div>
  ...
  <div style="display:flex;flex-direction:column;gap:16px">
    ...
    <div class="card">
      ...
      <div class="card-body" style="padding:0">
        <table style="width:100%;border-collapse:collapse;font-size:14px">
          <?php
          $infoRows = [
            ['예배 요일', $dept['worship_day']     ?? ''],
            ['예배 시간', $dept['worship_time']     ?? ''],
            ['예배 장소', $dept['worship_location'] ?? ''],
            ['연령대',    $dept['age_group']        ?? ''],
            ['사역 유형', $dept['ministry_type']    ?? ''],
          ];
    ...
    <div class="card">
      ...
      <div class="card-body" style="padding:0">
        <table style="width:100%;border-collapse:collapse;font-size:14px">
          <?php
          $clergRows = [
            ['담당자명', $dept['clergy_name']     ?? ''],
            ['직책',     $dept['clergy_position'] ?? ''],
            ['연락처',   $dept['clergy_phone']    ?? ''],
          ];
          foreach ($clergRows as [$label, $val]):
            if (!$val) continue;
          ?>
          <tr style="border-bottom:1px solid var(--border)">
    ...
    <div class="card">
      ...
      <div class="card-body" style="padding:0">
        ...
        <div style="padding:20px;text-align:center;color:var(--text-muted);font-size:13px">공지사항이 없습니다.</div>
        <?php else: ?>
        <table style="width:100%;border-collapse:collapse;font-size:13px">
          <thead>
            <tr>
              <th style="padding:8px 14px;background:var(--bg);text-align:left">제목</th>
              <th style="padding:8px 14px;background:var(--bg);text-align:left;white-space:nowrap">등록일</th>
            </tr>
          </thead>
          <tbody>
        ...
        <div style="padding:12px 14px;display:flex;gap:4px;flex-wrap:wrap">
          <?php if($pagination['has_prev']): ?>
          <a href="?id=<?= $dept['id'] ?>&page=<?= $pagination['current']-1 ?>" class="btn btn-ghost btn-sm">&laquo;</a>
          <?php endif; ?>
          ...
          <?php else: ?>
          <a href="?id=<?= $dept['id'] ?>&page=<?= $p ?>" class="btn btn-ghost btn-sm"><?= $p ?></a>
          <?php endif; ?>
          <?php endfor; ?>
          <?php if($pagination['has_next']): ?>
          <a href="?id=<?= $dept['id'] ?>&page=<?= $pagination['current']+1 ?>" class="btn btn-ghost btn-sm">&raquo;</a>
          <?php endif; ?>
        </div>
        <?php endif; ?>
        <?php endif; ?>
...
<div class="modal-overlay hidden" id="dept-modal">
  <div class="modal modal-xl">
    <div class="modal-header">
      <h3>부서 수정</h3>
      <button class="btn btn-ghost btn-icon" onclick="closeModal('dept-modal')"><i class="fas fa-times"></i></button>
    </div>
    <div class="modal-body">
      <div class="form-row">
        <div class="form-group">
          <label class="form-label">부서명 <span class="req">*</span></label>
          <input type="text" id="d-name" class="form-control" value="<?= htmlspecialchars($dept['name']) ?>">
        </div>
        <div class="form-group">
          <label class="form-label">유형</label>
          <select id="d-type" class="form-control">
            <option value="nextgen"  <?= $dept['department_type']==='nextgen'  ? 'selected' : '' ?>>다음세대</option>
            <option value="ministry" <?= $dept['department_type']==='ministry' ? 'selected' : '' ?>>사역부서</option>
          </select>
        </div>
        <div class="form-group">
          <label class="form-label">연령대</label>
          <input type="text" id="d-age" class="form-control" value="<?= htmlspecialchars($dept['age_group']??'') ?>">
        </div>
      </div>
      <div class="form-group">
        <label class="form-label">설명</label>
        <textarea id="d-desc" class="form-control" rows="3"><?= htmlspecialchars($dept['description']??'') ?></textarea>
      </div>
      <div class="form-row">
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <input type="file" id="d-img" class="form-control" accept="image/*">
          <?php if($dept['image']): ?>
          <div style="margin-top:6px"><img src="<?= UPLOAD_URL.htmlspecialchars($dept['image']) ?>" style="max-height:60px;border-radius:4px"></div>
          <?php endif; ?>
        </div>
        ...
        <div class="form-group">
          ...
          <select id="d-active" class="form-control">
    ...
    <div class="modal-footer">
      ...
      <button class="btn btn-primary" id="d-save-btn" onclick="saveDept()">저장</button>
    </div>
  </div>
</div>
...
async function saveDept() {
  ...
  const m = {
    department_type:'d-type', name:'d-name', description:'d-desc',
    age_group:'d-age', worship_day:'d-wday', worship_time:'d-wtime',
    worship_location:'d-wloc', clergy_name:'d-cname', clergy_position:'d-cpos',
    clergy_phone:'d-cphone', order:'d-order', is_active:'d-active'
  };
  for (const [k, eid] of Object.entries(m)) fd.append(k, document.getElementById(eid).value);
  const img = document.getElementById('d-img').files[0];
  if (img) fd.append('image', img);
  const btn = document.getElementById('d-save-btn'); btn.disabled = true;
  showSpinner('부서 정보 저장 중...');
  const d = await fetch(BASE_URL+'/departments/update', {method:'POST', body:fd}).then(r=>r.json());
  hideSpinner(); btn.disabled = false;
  if (d.success) { toast(d.message); closeModal('dept-modal'); location.reload(); }
